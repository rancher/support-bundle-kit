...
MK_DOCKER_PULL            ?= --pull
MK_TEST_INTEGRATION_IMAGE := sbk-test-integration:$(MK_REPO_ID)
...
test: gen-version-env
	...
	docker run $(MK_DOCKER_RUN_OPTS_TTY) --rm \
	    -v support-bundle-kit-test-go-cache-${MK_REPO_ID}:/go/src/github.com/rancher/support-bundle-kit/.cache/go-build \
	    $(MK_TEST_INTEGRATION_IMAGE) \
	    ./scripts/test


# ---- Package support-bundle-kit image ----
package: build
	$(BANNER)
